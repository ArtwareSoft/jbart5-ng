System.register(['js/jb', 'angular2/core', 'angular2/common', '/jbart/node_modules/ng2-material/all', 'ui/jb-rx', 'ui/dialog'], function(exports_1, context_1) {
    "use strict";
    ...
    var jb_1, core_1, common_1, all_1, jb_rx, jb_dialog;
    ...
    function Comp(options, context) {
        ...
        Cmp = Reflect.decorate([
            core_1.Component({
                ...
                directives: [all_1.MATERIAL_DIRECTIVES, common_1.FORM_DIRECTIVES, common_1.NgClass]
            }),
    ...
    function enrichComp(comp, ctrl_ctx) {
        ...
        jb_1.jb.extend(comp, { jbInitFuncs: [], jbBeforeInitFuncs: [], jbAfterViewInitFuncs: [], jbCheckFuncs: [], jbObservableFuncs: [] });
        ...
        var title = jb_tosingle(jb_1.jb.val(ctrl_ctx.params.title)) || (function () { return ''; });
        ...
        comp.jbExtend = function (options, context) {
            ...
            (options.styles || [])
                ...
                .forEach(function (x) { return jb_1.jb.path(options, ['atts', 'style'], x.match(/^{([^]*)}$/m)[1]); });
            overridable_props.forEach(function (prop) { return annotations[prop] = options[prop] || annotations[prop]; });
            extendable_array_props.forEach(function (prop) { return annotations[prop] = (annotations[prop] || []).concat(jb_1.jb.toarray(options[prop])); });
            options.atts = jb_1.jb.extend({}, options.atts, options.host); // atts is equvivalent to host
            if (options.cssClass)
                jb_1.jb.path(options, ['atts', 'class'], options.cssClass);
            ...
            Object.getOwnPropertyNames(options.atts || {})
                ...
                if (att == 'ngIf')
                    return jb_1.jb.path(annotations, ['host', 'template'], 'ngIf ' + val);
                if (att == 'class' && jb_1.jb.path(annotations, ['host', 'class']))
                    val = jb_1.jb.path(annotations, ['host', 'class']) + ' ' + val;
                if (att == 'style' && jb_1.jb.path(annotations, ['host', 'style']))
                    val = jb_1.jb.path(annotations, ['host', 'style']) + '; ' + val;
                jb_1.jb.path(annotations, ['host', att], val);
            });
        ...
        comp.jbApplyFeatures = function (featureProfiles, context) {
            (featureProfiles || []).forEach(function (f) { return comp.jbExtend(context.run(f), context); });
            return comp;
        };
        comp.jbCtrl = function (context) {
            ...
            jb_1.jb.path(options, ['atts', 'jb-path'], profilePath(context.profile) || ''); // for the studio
            ...
            if (context.params.style && context.params.style.profile && context.params.style.profile.features) {
                jb_1.jb.toarray(context.params.style.profile.features)
        ...
        function jbTemplate(options) {
            ...
            Array.from(template.attributes || [])
                ...
                if (ngAtt)
                    jb_1.jb.logError('ng atts are not allowed in root element of template: ' + att.name, { ctx: ctrl_ctx, att: att });
                return !ngAtt;
            })
                .forEach(function (att) { return host[att.name] = att.value; });
            jb_1.jb.extend(options, {
        ...
        function profilePath(profile) {
            ...
            if (lastFound && getPath(jb_1.jb.comps[lastFound].impl, profile, 0))
                return lastFound + '~' + getPath(jb_1.jb.comps[lastFound].impl, profile, 0).replace(/~*$/g, '');
            for (var comp in jb_1.jb.comps) {
                var impl = jb_1.jb.comps[comp].impl;
    ...
    function twoWayBind(ref) {
        if (ref.$jb_parent) {
            ...
            var bindToCmp = function (cmp) {
                cmp.writeValue = function (val) {
                    return jb_1.jb.writeValue(ref, val);
                };
                cmp.$jbModel = jb_1.jb.val(ref);
                jb_rx.refObservable(ref, cmp.ctx).subscribe(function () {
                    return cmp.$jbModel = jb_1.jb.val(ref);
                });
            };
        ...
        return {
            ...
            getValue: function () { return jb_1.jb.val(ref); },
            writeValue: function (val) { return jb_1.jb.writeValue(ref, val); }
        };
    }
            ...
            jBartWidget = (function () {
                ...
                jBartWidget.prototype.initRedrawEm = function () {
                    ...
                    function relevantSource(compID) {
                        ...
                        return Object.getOwnPropertyNames(jb_1.jb.comps).filter(function (id) { return id.indexOf(ns + '.') == 0; }).map(function (id) { return jb_1.jb.prettyPrint(jb_1.jb.comps[id].impl); }).join('');
                    }
                };
                jBartWidget.prototype.getOrCreateInitialCtx = function () {
                    ...
                    var ctx = jb_1.jb.ctx({ ngMode: true, resources: resources, vars: { ngZone: this.ngZone } }, {});
